...
def train_the_NIROM(nirom_options, fwd_options, U_encoded, ae=False):
    ...
    elif nirom_options.compression.autoencoder == True:
        t = 0
        nPOD_total = 0
 
        for ifield in range(len(nirom_options.compression.field)):
            nPOD_total = nPOD_total + nirom_options.compression.nLatent[ifield]
    ...
    if nParameters==1:
        ...
        if nirom_options.compression.autoencoder == True:
            nPOD = nirom_options.compression.nLatent
            decoder = load_model('decoder.h5') 
            U_decoded = decoder.predict(pod_coeffs_all_prediction) ##### check shape of this
            print "U_decoded shape", U_decoded.shape
            pod_coeffs_all_prediction = U_decoded #U_decoded.T
            solution_pred = pod_coeffs_all_prediction
