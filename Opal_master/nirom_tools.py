...
from dd_extract_data import generate_all_matrix_dd
         ...
         for iTime in range(nTime):
            ...
            if iTime==0 and p==0:
                if values.size != values.shape[0]:
                    nComponents = 2 # fwd_options.ndim   # hard code
                    field_values = values
                else:
                    nComponents = 1
...
def compress_snapshots_real_dd(interpolate_method, file_prefix, nLatent, dim, epochs, directory, nsplt, fwd_options, nirom_options):
    t = 0
    nPOD_total = 0
    filename = file_prefix +str(0) + ".vtu"
    vtu_data, coords = get_coordinates(filename)

    structured_shape = None
    if nsplt > 0:
        try:
            witchd=np.load("witchd_" + str(2**nsplt) + ".npy")
        except IOError:
            witchd = get_witchd(nsplt, coords)
            np.save("witchd_" + str(2**nsplt), witchd)

        structured_shape, min_all, max_all = get_domain_info(interpolate_method, nsplt, witchd, coords, dim=dim)
    elif nsplt == 0:
        witchd = [1 for i in range(coords.shape[0])]
        min_all = [[np.min(coords[:, i]) for i in range(dim)]]
        max_all = [[np.max(coords[:, i]) for i in range(dim)]]
        ...
        print("nsplt cannot be negative")
        sys.exit(1)
    ...
    try:
        for i in range(2**nsplt):
            data.append(np.load(directory+'/interpolated_matrix_' + str(i) + '.npy'))
    except IOError:
        DD_interpolation_from_vtu(directory, nsplt, coords, structured_shape, min_all, max_all, file_prefix=file_prefix, range_vtu=2000, dim=dim)
    ...
    try:
        ...
        print("Because of some bug on Linux version Keras, Please copy the interpolated matrix to other OS to train the auto-encoder.")
        sys.exit(0)

        print("start training autoencoder!")
        fig_num = 5
        ae = Autoencoder_interpolation(nLatent=nLatent, img_rows=[data[i].shape[1] for i in range(2**nsplt)],
                                   img_cols=[data[i].shape[2] for i in range(2**nsplt)], ndomain=2**nsplt)
        ae.train_model(data, epochs=epochs, batch_size=128)

        ae.encoder_model.save(directory + "/encoder_dd" + str(2**nsplt) + ".h5")
        ae.decoder_model.save_weights(directory+"/decoder_dd" + str(2**nsplt) + ".h5")
        encoder = ae.encoder_model

    data_a = [[] for i in range(len(data))]
    for j in range(len(data)):
        for i in range(0, data[0].shape[0], 20):
            data_a[j].append(data[j][i])
    data = data_a
...
def train_the_NIROM(nLatent, generate_directory, file_prefix, dim, snapshot_directory, witchd, structured_shape, min_all, max_all, nirom_options, fwd_options, U_encoded, ae=False, dd=True):
    ...
    print(nirom_options.compression.field)


    if nirom_options.compression.svd_autoencoder == True or nirom_options.compression.autoencoder == True:
        print("nPOD : ", len(nirom_options.compression.nPOD))
        nPOD_total =  32   #nirom_options.compression.nPOD[0]   # Todo : hard code
        ...
        for ifield in range(len(nirom_options.compression.field)):
            print("nPOD, ", ifield, "value : ", nirom_options.compression.nPOD[ifield])
            nPOD_total = nPOD_total + nirom_options.compression.nPOD[ifield]
    ...
    print("pod coeffs after transforrm", pod_coeffs_all.shape)
    ...
    if nParameters==1:
        ...
        if nirom_options.compression.autoencoder == True:
            print("shape of pod_all", pod_coeffs_all_prediction.shape)

            # todo:hard code
            if dd:
                preds = predict_dd(nLatent, file_prefix, snapshot_directory, structured_shape, encoded=pod_coeffs_all_prediction)
                ...
                predict_and_output_vtu(data=pod_coeffs_all_prediction, directory=generate_directory, file_prefix="nirom_prediction_")
